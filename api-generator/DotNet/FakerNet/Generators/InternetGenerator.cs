...
#warning Failed processing method device_token
        ...
        public string Email(string? name = null, string separators = "-_", string? domain = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(name))
                namePart = Username(5, 15, separator);
            else
        ...
        {
            return string.Join(".", Random.NextItem(PrivateIP4AddressRanges).Select(r => Random.NextInt32((int)r.Min, (int)r.Max).ToString(CultureInfo.InvariantCulture)));
        }
        #endregion
        ...
        public string Username(long minlen = 4, long maxlen = 12, string separators = "._")
        {
            ...
            int targetLen = Random.NextInt32((int)minlen, (int)maxlen);
            ...
            while (userName.Length <= targetLen)
            {
                ...
                if (firstPass == false && remainder < 4 && userName.Length >= (int)minlen && userName.Length <= (int)maxlen)
                    break; // close enough
            ...
            {
                int maxLen = (int)maxlen - userName.Length;
