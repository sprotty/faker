...
output('using LiquidTechnologies.MetaTypeSystem;\n');
output('using FakerNet;\n');
...
output('\n');
output('namespace LiquidTechnologies.SampleDataGeneratorModel\n');
output('{\n');
outputIndentPush();
{
    ...
    // output('[GeneratedCode("Faker Adapter Code Generator", "1.0.0.0")]\n');
...
fs.writeFileSync('./FakerMethodsAdapter.cs', output_data.join('').replaceAll('\r', ''));
...
function generateClass(cls, class_path) {
    ...
    output('[GeneratedCode("Faker Code Generator", "1.0.0.0")]\n');
    output('[NativeContainer]\n');
    output('public partial class ' + nativeClsName + '\n');
    output('{\n');
    outputIndentPush();
    {
        // output('internal ' + nativeClsName + '(Faker faker)\n');
        // output('         : base(faker)\n');
        // output('{\n');
        // outputIndentPush();
        // {
...
function writeMethod(method, class_path) {
    ...
    if (impl == null) {
        output('#warning No implementation defined for method ' + method.name + '\n');
        return;
    }

    const fakerMethodName = GetNativeMethodName(method.name);
    const nativeMethodName = isNullOrWhitespace(method.lttdg_name) ? fakerMethodName : method.lttdg_name;
    ...
    output('[NativeFunction("' + getNativeEscapedString(nativeMethodName) + '", "' + getNativeEscapedString(methodDesc) + '", "' + class_path.map(g => getNativeEscapedString(g)).join('.') + '", "function-' + class_path.map(g => g.toLowerCase()).join('-') + '-' + nativeMethodName + '.png", AccessorType.Method)]\n');
    output('[return: ArgumentProperty("Value", "' + getNativeEscapedString(returnValueDesc) + '")]\n')
    output('public static ' + nativeMethodType + ' ' + nativeMethodName + '(\n');
    outputIndentPush();
    {
        output('  [ArgumentProperty("context", "internal use")] IEvaluationContext context');
        output(getArgs(method.arguments));
    ...
    outputIndentPush();
    if (method.usesLocale != false)
        output('return context.GetFakerInstance(locale).');
    else
        output('return context.GetFakerInstance(new CultureInfo("en")).');
    output(class_path.join('.') + '.' + fakerMethodName + '(' + method.arguments.map(a => getArgValue(a)).join(', ') + ');\n');
...
function getArg(arg) {
    ...
    var nameParam = '"' + getNativeEscapedString(argNativeName) + '"';
    var descParam = ', "' + getNativeEscapedString(arg.text ?? '') + '"';
    var argDefaultDecl = '';
    var optionalParam = '';
    ...
    if (isNullOrWhitespace(arg.default) == false) {
        ...
        optionalDescParam += ', OptionalDesc = "' + getNativeEscapedString(arg.default) + '"';
        ...
        if (arg.default == 'nil') {
            ...
            argDefaultDecl = ', Default = "' + getNativeEscapedString(arg.default.substring(1, arg.default.length - 1)) + '"';
        }
